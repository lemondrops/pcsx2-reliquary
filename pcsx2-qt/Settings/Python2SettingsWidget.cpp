...
Python2SettingsWidget::Python2SettingsWidget(const GameList::Entry* entry, SettingsWindow* window, QWidget* parent)
	: m_window(window)
{
	...
	const int gameTypeId = m_window->getIntValue("Python2/Game", "GameType", 0).value();
	...
	SettingWidgetBinder::BindWidgetToStringSetting(sif, m_ui.ilinkIdPath, "Security", "ILinkIdFile", "ILINK_ID.bin");
	...
	const std::string deviceApi = m_window->getStringValue("Python2/Game", "DeviceApi", "native").value();
...
{
	m_window->setIntSettingValue("Python2/Game", "GameType", index);
}

void Python2SettingsWidget::onDeviceApiChange(int index)
{
	auto deviceApi = m_ui.deviceApi->currentData().toString().toStdString();
	m_window->setStringSettingValue("Python2/Game", "DeviceApi", deviceApi.c_str());
}
